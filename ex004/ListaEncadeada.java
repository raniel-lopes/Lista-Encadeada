...
public class ListaEncadeada {
	...
	public void adicionar(Professor valor) {
		...
		// Condição para saber como está a leite
		if (primeiro == null && ultimo == null) {
			//Entro apenas se a lista estiver vazia
			primeiro = celula;
			ultimo = celula;
		} else {
			// Entra quando a lista já conter um dado
	...
	// Metodo para se movimentar na sequencia das casinhas
		...
		//  Método que inicia a busca recursiva
		public Professor buscarRecursivo(int id) {
			return buscarRecursivoInterno(primeiro, id);
		}
		
		// Método privado que faz a busca recursiva
		private Professor buscarRecursivoInterno(Celula atual, int id) {
			// Caso base: chegou ao fim da lista ou lista vazia
			if(atual == null) {
				return null;
			}
			
			// Se encontrou o professor com o ID
			if  (atual.getValor().getId() == id) {
				return atual.getValor();
			}
			
			// Chamada recursiva para o próximo=
			return buscarRecursivoInterno(atual.getProximo(), id);
		}
